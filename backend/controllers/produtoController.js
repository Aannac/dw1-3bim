...
exports.criarProduto = async (req, res) => {
  ...
  try {
    const { nome_produto, preco_unitario, quantidade_estoque } = req.body;
    ...
    if (isNaN(preco_unitario) || isNaN(quantidade_estoque)) {
    ...
    const result = await query(
      "INSERT INTO produto (nome_produto, preco_unitario, quantidade_estoque, imagem_produto) VALUES ($1, $2, $3, $4) RETURNING *",
      [nome_produto, preco_unitario, quantidade_estoque, imagemProduto]
    );
...
exports.atualizarProduto = async (req, res) => {
  ...
  try {
    ...
    const updatedFields = {
      ...
      preco_unitario: preco_unitario !== undefined ? preco_unitario : currentProduct.preco_unitario,
    ...
    const updateResult = await query(
      `UPDATE produto 
       SET nome_produto = $1, preco_unitario = $2, quantidade_estoque = $3, imagem_produto = $4
       WHERE id_produto = $5 RETURNING *`,
      [updatedFields.nome_produto, updatedFields.preco_unitario, updatedFields.quantidade_estoque, 
        updatedFields.imagem_produto, id]
    );
